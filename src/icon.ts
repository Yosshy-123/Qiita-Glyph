 ...
 *
 * Limitation: this only inspects the literal hostname in the URL. It does
 * not resolve DNS, so a hostname that *resolves* to a private address
 * (DNS rebinding) would not be caught here. That risk is accepted because
 * the URL originates from Qiita's own API response, not directly from an
 * untrusted caller.
 */
